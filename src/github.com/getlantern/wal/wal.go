...
func (fb *filebased) openFile() error {
	...
	if fb.file != nil {
		err = fb.file.Close()
		if err != nil {
			logger.Error("Unable to close existing file %v: %v", fb.file.Name(), err)
		}
	}
	...
	if err == nil {
		...
		logger.Debug("Opened %v (%v)", filename, ts)
	}
	return err
}
...
func (wal *WAL) TruncateBefore(o Offset) error {
	...
	return wal.forEachSegment(func(file os.FileInfo, first bool, last bool) (bool, error) {
		...
		logger.Debug("Removed WAL file %v", filepath.Join(wal.dir, file.Name()))
		return true, nil
	})
}
...
func (wal *WAL) TruncateToSize(limit int64) error {
	...
	return wal.forEachSegmentInReverse(func(file os.FileInfo, first bool, last bool) (bool, error) {
		...
		if seen > limit {
			...
			logger.Debug("Removed WAL file %v", fullname)
		}
		return true, nil
	})
}
...
func (wal *WAL) compress(file os.FileInfo) (bool, error) {
	...
	logger.Debug("Compressed WAL file %v", infile)
	return true, nil
}
...
func (wal *WAL) doSync() {
	err := wal.writer.Flush()
	if err != nil {
		logger.Error("Unable to flush wal: %v", err)
		return
	}
	err = wal.file.Sync()
	if err != nil {
		logger.Error("Unable to sync wal: %v", err)
	}
}
...
func (wal *WAL) NewReader(name string, offset Offset) (*Reader, error) {
	...
	if offset != nil {
		...
		if offsetString[0] != '0' {
			logger.Debug("Converting legacy offset")
	...
	if r.file == nil {
		...
		logger.Debug("Replaying log starting at %v", r.file.Name())
	}
	return r, nil
}
		...
		if data != nil || err != nil {
			if data != nil {
				...
				if checksum != int(r.h.Sum32()) {
					logger.Error("Checksum mismatch, skipping entry")
					continue
				}
			}
			return data, err
		...
		for {
			...
			if err != nil && err.Error() == "EOF" && read < 4 {
				if r.wal.hasMovedBeyond(r.fileSequence) {
					if read > 0 {
						logger.Error("Out of data to read after reading %d, and WAL has moved beyond %d. Assuming WAL at %v corrupted. Advancing and continuing.", r.position, r.fileSequence, r.filename())
					}
			...
			if err != nil {
				logger.Error("Unexpected error reading header from WAL file %v: %v", r.filename(), err)
				break
			}
			if read == 4 {
...
func (r *Reader) readData(length int) ([]byte, error) {
	...
	for {
		...
		if err != nil {
			logger.Error("Unexpected error reading data from WAL file %v: %v", r.filename(), err)
			return nil, nil
		}
...
func (r *Reader) advance() error {
	logger.Debug("Advancing in %v", r.dir)
	for {
		if atomic.LoadInt32(&r.closed) == 1 {
